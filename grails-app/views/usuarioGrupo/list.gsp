			...
			<table>
				...
				<g:each in="${usuarioGrupoInstanceList}" status="i" var="usuarioGrupoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${usuarioGrupoInstance.id}">${fieldValue(bean: usuarioGrupoInstance, field: "grupo")}</g:link></td>
					
						<td>${fieldValue(bean: usuarioGrupoInstance, field: "usuario")}</td>
